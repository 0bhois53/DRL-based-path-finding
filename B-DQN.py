...
os.makedirs('bdQN_visuals', exist_ok=True)
...
if __name__=="__main__":
    import time
    import psutil
    # Set latent_dim and beta for β-VAE bottleneck
    latent_dim = 32
    beta = 4.0
    agent = BDQNAgent.DQNAgent(state_space_dim, n_actions, replay_buffer_size, batch_size,
                 hidden, gamma, latent_dim=latent_dim, beta=beta)
    random.seed(20)
    env.reset()
    ...
    os.makedirs('bdqn_saved_models', exist_ok=True)
    ...
    for ep in range(num_episodes):
        ...
        while not done and counter < env.max_episode_steps:
            # Flatten state before passing to agent
            flat_state = state.flatten() if hasattr(state, 'flatten') else np.array(state).flatten()
            ...
            agent.store_transition(flat_state, action, next_state.flatten() if hasattr(next_state, 'flatten') else np.array(next_state).flatten(), reward, done)
        ...
        if done:
            ...
            if cum_reward > best_reward:
                ...
                agent.save_model(f'bdqn_saved_models/bdqn_best_model.pth')
    ...
    # Save final model
    agent.save_model('bdqn_saved_models/bdqn_final_model.pth')
    print("Training completed. Final model saved to 'bdqn_saved_models/bdqn_final_model.pth'")

    # Evaluate the trained model
    def evaluate_model(agent, env, num_eval_episodes=10, model_path='bdqn_saved_models/bdqn_final_model.pth'):
        ...
        for eval_ep in range(num_eval_episodes):
            ...
            while not done and steps < env.max_episode_steps:
                ...
                flat_state = state.flatten() if hasattr(state, 'flatten') else np.array(state).flatten()
                action = agent.get_action(flat_state, epsilon=0.0)
                next_state, next_state_flag, reward, done, _ = env.step(action)
...
# Print additional training parameters
    ...
    plt.xticks(size = '12')
    plt.yticks(size = '12')
    plt.savefig('bdQN_visuals/b-DQN_Accumulated_Reward.png', format='png', dpi=300)
    ...
    plt.xticks(size = '12')
    plt.yticks(size = '12')
    plt.savefig('bdQN_visuals/b-DQN_Steps_per_Episode.png', format='png', dpi=300)
    ...
    plt.savefig('bdQN_visuals/b-DQN_Shortest_Path.png', format='png', dpi=300)
    ...
    plt.yticks(size = '12')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig('bdQN_visuals/b-DQN_Final_Path.png', format='png', dpi=300)
    plt.show()
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] == '--animate':
                ...
                anim, results = animate_evaluation(agent, env, starting_position, target_position,
                                                 model_path='bdqn_saved_models/bdqn_final_model.pth',
