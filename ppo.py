...
class PPOAgent:
	def __init__(self, state_dim, action_dim, lr=2e-4, gamma=0.98, clip=0.2, update_epochs=15, device=None):
...
def train_ppo(env, agent, episodes=500, max_steps=300, batch_size=128, save_path='PPO_runs/ppo_gridworld.pt'):
	...
	for ep in range(episodes):
		...
		if len(memory) >= batch_size:
			...
			for _ in range(agent.update_epochs):
				...
				loss = actor_loss + 0.5 * critic_loss - 0.045 * entropy.mean()
	...
	plt.xticks(size = '12')
	plt.yticks(size = '12')
	plt.savefig('PPO_runs/ppo_training_curve.png', format='png', dpi=300)
	plt.close()
	...
	plt.xticks(size = '12')
	plt.yticks(size = '12')
	plt.savefig('PPO_runs/ppo_steps_per_episode.png', format='png', dpi=300)
	plt.close()
	...
	if best_path:
		plot_path_with_obstacles(best_path, 'PPO_runs/ppo_shortest_path.png', 'Shortest Steps Path', env)
	# Final path visualization
	if final_path:
		plot_path_with_obstacles(final_path, 'PPO_runs/ppo_final_path.png', 'Final Path', env)
