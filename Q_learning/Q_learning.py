...
def load_selected_points(filename=r'T:\PPO\DRL-based-path-finding\Q_learning\selected_points.txt'):
    try:
...
def update():
    ...
    # Save final model
    agent.save_model(r'T:\PPO\DRL-based-path-finding\Q_learning\saved_models_Q_learning\q_learning_final_model.pkl')
    ...
    plt.xticks(size = '12')
    plt.yticks(size = '12')
    plt.savefig(r'T:\PPO\DRL-based-path-finding\Q_learning\Q_learning_visuals\Q_learning_Steps_per_Episode.png', format='png', dpi=300)
    ...
    plt.savefig(r'T:\PPO\DRL-based-path-finding\Q_learning\Q_learning_visuals\Q_learning_Shortest_Path.png', format='png', dpi=300)
    ...
    for i in range(len(env.Obstacle_x)):
        rectangle = mpatches.Rectangle((10 * (env.Obstacle_x[i] - 0.5), 10 * (10 - env.Obstacle_y[i] - 0.5)), 
    ...
    plt.yticks(size = '12')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig('qlearning_Shortest_Path_unsmoothed.png', format='png', dpi=300)
    plt.show()
...
def evaluate_model(agent, env, num_eval_episodes=10, model_path=r'T:\PPO\DRL-based-path-finding\Q_learning\saved_models_Q_learning\q_learning_final_model.pkl', 
