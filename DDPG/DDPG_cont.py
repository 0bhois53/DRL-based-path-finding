...
class HERReplayBuffer:
	def __init__(self, max_size=200000, her_k=4):
		...
		self.success_tol = 0.5  # Default tolerance for success
		...
		for idx, trans in enumerate(ep):
			...
			if idx + 1 < L:
				for _ in range(self.her_k):
					...
					achieved = ep[future_idx][3] 
					new_goal = achieved[:2].copy() 
...
class DDPGAgent:
	def __init__(self, state_dim, action_dim, max_action=0.5, device=None, success_tol=0.5):
	...
	def train(self, batch_size=128, policy_noise=0.2, noise_clip=0.2, policy_freq=2):
...
def train_loop(episodes=500, max_steps=150, render=False):
